...
def train_GMMs(features, target_labels):
    ...
    for gmm_id in range(NUM_VOWELS):
        #for vowel_id in target_labels:
        gmm_set[gmm_id].fit(vowel_features)
        print()
    return gmm_set
...
def eval_GMMs(gmm_set, features, target_labels):
    ...
    rec_labels = []
    for vowel_id in target_labels:
        scores = [gmm_set[gmm_id].score(features) for gmm_id in range(NUM_VOWELS)]
        rec_labels = np.append(rec_labels, np.argmax(scores))
	# Compute accuracy
